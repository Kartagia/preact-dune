import {html,h} from 'preact';
import {ComponentChildren, FunctionComponent } from 'preact/compat';

export function toChildArray(children: ComponentChildren): ComponentChild[] {
...
export type IconProps = IconProperties & IconEvents;
...
export function IconButton(props: IconProps & {children: ComponentChildren}) {
  const {children, ...iconProps} = props;
  const icon = [...toChildArray(props.children)].find(
    cursor => (cursor.type === "i"))?.props?.children?.trim();
  console.log(`Icon ${icon ?? `No icon for ${JSON.stringify(children)}`}`);
    
  if (icon && icons.has(icon)) {
    return icons.get(icon)?.(iconProps);
  } else {
    return html`<div></div>`;
  }
}