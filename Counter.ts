...
export function Counter() {
  const [input, parsed, partial, setInput, setParsed] = useParsedValue<number>({
    ...
    parser: (a: string) => Number.parseInt(a)
  });
  const decrease = () => {
    setParsed(parsed - 1)
  };
...
export function CounterControl<T>(props: Omit<CounterProps<T>, "defaultValue"> & {
  ...
  const compare: Compare<T> = props.compare ?? ((compared: T, comparee: T) => defaultCompare(compared, comparee));
  console.table({
    value: props.value,
  ...
  if (
    ("min" in props && props.min !== undefined && compare(props.min, props.value) > 0) ||
    ("max" in props && props.max !== undefined && compare(props.value, props.max) > 0)
  ) {
...
export function InputCounter<T>(props: CounterProps<T>) {
  ...
  const parser = props.parse ?? ((val: string) => { throw new Error("Parse not supported"); });
  ...
  const confirmed = (e) => {
    if (typeof e === "object") {
      ...
      try {
        setParsed(parser(e.target.value))
      } catch (error) {
        setParsed(parsed);
      }
    }
